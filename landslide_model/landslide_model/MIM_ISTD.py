...
class PyramidMiM_enc(nn.Module):
    ...
    def __init__(self, configs=None, img_size=128, in_chans=4, num_classes=1, mlp_ratio=4., qkv_bias=False,
...
class MiM(nn.Module):
    def __init__(self, layer_blocks, channels, in_channels):
        ...
        self.head = _FCNHead(channels[1], 2)
        #####################
        self.mim_backbone = PyramidMiM_enc(in_chans=self.in_channels)
...
if __name__ == '__main__':
    input_ = torch.Tensor(5, 14, 128, 128)
    net = MiM([2] * 3, [16, 32, 64, 128, 256])
    out = net(input_)
    print(out.shape)