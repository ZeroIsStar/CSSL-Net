...
class UNet_3Plus1(nn.Module):

    def __init__(self, in_channels=64, feature_scale=4, is_deconv=True, is_batchnorm=True):
        super(UNet_3Plus1, self).__init__()
...
class UNet_3Plus2(nn.Module):

    def __init__(self, in_channels=128, feature_scale=4, is_deconv=True, is_batchnorm=True):
        super(UNet_3Plus2, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
    ...
    def forward(self, inputs):
        ...
        hd3_UT_hd2 = self.hd3_UT_hd2_relu(self.hd3_UT_hd2_bn(self.hd3_UT_hd2_conv(self.hd3_UT_hd2(hd3))))
        hd2 = self.relu2d_1(self.bn2d_1(self.conv2d_1(
            torch.cat((h1_PT_hd2, h2_Cat_hd2, hd3_UT_hd2), 1))))
...
class ResNet3(nn.Module):
    def __init__(self, block, layers):
        ...
        self.conv1 = nn.Conv2d(320, 256, kernel_size=3, stride=1, padding=1, bias=False)
...
class MixVisionTransformer(nn.Module):
    def __init__(self, in_chans=14, num_classes=1000, embed_dims=[32, 64, 160, 256],
...
class mit_b0(MixVisionTransformer):
    def __init__(self, pretrained=False):
        ...
        if pretrained:
            ...
            self.load_state_dict(torch.load("model_data/weights.pth"), strict=False)
